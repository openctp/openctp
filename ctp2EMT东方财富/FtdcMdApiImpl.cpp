...
void CFtdcMdApiImpl::HandleReqUserLogin(CThostFtdcReqUserLoginField& ReqUserLogin, int nRequestID)
{
	...
	tt = time(NULL);
	t = localtime(&tt);
	strncpy(TradingDay, "", sizeof(TradingDay) - 1);
...
///行情订阅应答
void CFtdcMdApiImpl::OnSubMarketData(EMTQuoteStaticInfo* ticker, EMTRspInfoStruct* error_info, bool is_last)
{
	if(m_pSpi)
	{
...
///行情退订应答
void CFtdcMdApiImpl::OnUnSubMarketData(EMTQuoteStaticInfo* ticker, EMTRspInfoStruct* error_info, bool is_last)
{
	if (m_pSpi)
	{
...
///行情退订应答
void CFtdcMdApiImpl::OnDepthMarketData(EMTMarketDataStruct* market_data, int64_t bid1_qty[], int32_t bid1_count, int32_t max_bid1_count, int64_t ask1_qty[], int32_t ask1_count, int32_t max_ask1_count)
{
	if (m_pSpi)
	{
		...
		ThostDepthMarketData.OpenInterest = 0;
		...
		ThostDepthMarketData.PreOpenInterest = 0;
