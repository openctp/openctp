...
{
	m_pTimer = new boost::asio::deadline_timer(m_io_service, boost::posix_time::seconds(EMT_RECONNECT_TIME)); // 毫秒
...
{
	m_pUserApi->Logout(m_emt_sessionid);
	if (m_pSpi) {
...
void CFtdcTraderApiImpl::HandleReqOrderInsert(CThostFtdcInputOrderField& InputOrder, int nRequestID)
{
	...
	if (strcmp(InputOrder.ExchangeID, "SSE") == 0) {
		Req.market = EMT_MKT_SH_A;
	}
	else if (strcmp(InputOrder.ExchangeID, "SZSE") == 0) {
		Req.market = EMT_MKT_SZ_A;
	}

	strncpy(Req.ticker, InputOrder.InstrumentID,sizeof(Req.ticker)-1);
	Req.order_client_id = atol(InputOrder.OrderRef);
	Req.price_type = InputOrder.OrderPriceType == THOST_FTDC_OPT_AnyPrice ? EMT_PRICE_BEST5_OR_CANCEL : EMT_PRICE_LIMIT;
	Req.side = InputOrder.Direction== THOST_FTDC_D_Buy? EMT_SIDE_BUY : EMT_SIDE_SELL;
	Req.price = InputOrder.LimitPrice;
	Req.quantity = InputOrder.VolumeTotalOriginal;
	Req.business_type = EMT_BUSINESS_TYPE_CASH;
	Req.position_effect = EMT_POSITION_EFFECT_INIT;

	uint64_t emt_order_id = m_pUserApi->InsertOrder(&Req, m_emt_sessionid);
	if (emt_order_id == 0) {
		// 下单失败
		if (m_pSpi) {
	...
	m_mOrderRefToXtpID[ctp_order_id] = emt_order_id;
	m_mXtpIDToOrderRef[emt_order_id] = ctp_order_id;
}
...
{
	uint64_t emt_order_id = atoll(InputOrderAction.OrderSysID);
	if (emt_order_id == 0) {
		...
		if (iter == m_mOrderRefToXtpID.end())
			return; // 找不到订单
		emt_order_id = iter->second;
	}
	if (m_pUserApi->CancelOrder(emt_order_id, m_emt_sessionid) == 0) {
		// 撤单失败
		if (m_pSpi) {
			CThostFtdcRspInfoField RspInfo;
...
void CFtdcTraderApiImpl::OnCancelOrderError(EMTOrderCancelInfo* cancel_info, EMTRI* error_info, uint64_t session_id)
{
	if (error_info && error_info->error_id != 0) {
		...
		if (cancel_info) {
			strncpy(OrderAction.OrderSysID, std::to_string(cancel_info->order_emt_id).c_str(),sizeof(OrderAction.OrderSysID));
			OrderAction.OrderActionRef = cancel_info->order_cancel_emt_id;
...
int CFtdcTraderApiImpl::ReqQryOrder(CThostFtdcQryOrderField *pQryOrder, int nRequestID)
{
	...
	return m_pUserApi->QueryOrders(&Req, m_emt_sessionid, nRequestID);
}
...
{	
	...
	return m_pUserApi->QueryTrades(&Req, m_emt_sessionid, nRequestID);
}
...
{
	return m_pUserApi->QueryAsset(m_emt_sessionid, nRequestID);
}

//资金账户查询
void CFtdcTraderApiImpl::OnQueryAsset(EMTQueryAssetRsp* trading_account, EMTRI* error_info, int request_id, bool is_last, uint64_t session_id)
{
	if (m_pSpi)
	{
		CThostFtdcTradingAccountField* pCTPTradingAccount = NULL;
		CThostFtdcTradingAccountField CTPTradingAccount;
		if (trading_account &&  trading_account->account_type == EMT_ACCOUNT_NORMAL)
		{
...
//投资者持仓查询
void CFtdcTraderApiImpl::OnQueryPosition(EMTQueryStkPositionRsp* position, EMTRI* error_info, int request_id, bool is_last, uint64_t session_id)
{
	if (m_pSpi)
	{
		CThostFtdcRspInfoField* pThostRspInfo = NULL;
		...
		if (position)
		{
			...
			ThostRspInvestorPosition.PositionCost = position->avg_price;
			...
			ThostRspInvestorPosition.OpenCost = position->avg_price;
...
//报单查询
void CFtdcTraderApiImpl::OnQueryOrder(EMTQueryOrderRsp* order_info, EMTRI* error_info, int request_id, bool is_last, uint64_t session_id)
{
	if (m_pSpi)
	{
		...
		if (order_info)
		{
			...
			ThostOrder.OrderPriceType = order_info->order_type == EMT_PRICE_LIMIT ? THOST_FTDC_OPT_LimitPrice : THOST_FTDC_OPT_AnyPrice;
			ThostOrder.Direction = order_info->side == EMT_SIDE_BUY ? THOST_FTDC_D_Buy : THOST_FTDC_D_Sell;
			ThostOrder.CombOffsetFlag[0] = order_info->side == EMT_SIDE_BUY ? THOST_FTDC_OF_Open : THOST_FTDC_OF_Close;
			ThostOrder.CombHedgeFlag[0] = THOST_FTDC_HF_Speculation;
			ThostOrder.FrontID = 0;
			ThostOrder.SessionID = 0;
			strncpy(ThostOrder.OrderSysID, std::to_string(order_info->order_emt_id).c_str(), sizeof(ThostOrder.OrderSysID) - 1);
			uint64_t ctp_order_id;
			uint32_t OrderRef;
			auto iter = m_mXtpIDToOrderRef.find(order_info->order_emt_id);
			if (iter == m_mXtpIDToOrderRef.end()) {
				OrderRef = m_ctp_sys_orderref++;
				ctp_order_id = OrderRef; // 系统生成的OrderRef对应的SessionID为0
				m_mXtpIDToOrderRef[order_info->order_emt_id] = ctp_order_id;
				m_mOrderRefToXtpID[ctp_order_id] = order_info->order_emt_id;
			}
			else {
			...
			strncpy(ThostOrder.OrderLocalID, std::to_string(order_info->order_emt_id).c_str(), sizeof(ThostOrder.OrderLocalID) - 1);
...
//成交查询
void CFtdcTraderApiImpl::OnQueryTrade(EMTQueryTradeRsp* trade_info, EMTRI* error_info, int request_id, bool is_last, uint64_t session_idt)
{
	if (m_pSpi)
	{
		...
		if (trade_info)
		{
			...
			strncpy(ThostTrade.OrderSysID, std::to_string(trade_info->order_emt_id).c_str(), sizeof(ThostTrade.OrderSysID) - 1);
			uint64_t ctp_order_id;
			uint32_t OrderRef;
			auto iter = m_mXtpIDToOrderRef.find(trade_info->order_emt_id);
			if (iter == m_mXtpIDToOrderRef.end()) {
				OrderRef = m_ctp_sys_orderref++;
				ctp_order_id = OrderRef; // 系统生成的OrderRef对应的SessionID为0
				m_mXtpIDToOrderRef[trade_info->order_emt_id] = ctp_order_id;
				m_mOrderRefToXtpID[ctp_order_id] = trade_info->order_emt_id;
			}
			else {
				OrderRef = iter->second << 32 >> 32;
			}
			snprintf(ThostTrade.OrderRef, sizeof(ThostTrade.OrderRef), "%u", OrderRef);
			ThostTrade.Direction = trade_info->side == EMT_SIDE_BUY ? THOST_FTDC_D_Buy : THOST_FTDC_D_Sell;
			ThostTrade.OffsetFlag = trade_info->side == EMT_SIDE_BUY ? THOST_FTDC_OF_Open : THOST_FTDC_OF_Close;
			ThostTrade.HedgeFlag = THOST_FTDC_HF_Speculation;
			ThostTrade.Price = trade_info->price;
			ThostTrade.Volume = trade_info->quantity;
			strncpy(ThostTrade.OrderLocalID, std::to_string(trade_info->order_emt_id).c_str(), sizeof(ThostTrade.OrderLocalID) - 1);
...
//报单回报
void CFtdcTraderApiImpl::OnOrderEvent(EMTOrderInfo* order_info, EMTRI* error_info, uint64_t session_id)
{
	if (m_pSpi) {
		CThostFtdcOrderField ThostOrder;
		memset(&ThostOrder, 0x00, sizeof(ThostOrder));
...
//成交回报
void CFtdcTraderApiImpl::OnTradeEvent(EMTTradeReport* trade_info, uint64_t session_id)
{
	if (m_pSpi)
	{
		CThostFtdcTradeField  ThostTrade;
