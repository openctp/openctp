...
ATPClientFeatureCodeType g_client_feature_code = "PC;IIP=101.86.234.47;IPORT=12345;LIP=127.0.0.1;MAC=00:0c:29:68:56:f9;HD=NA;PCN=NA;CPU=NAPI=NA;VOL=NA@V0.0.0.1";  // 终端识别码，由券商指定规则
...
ATPRetCodeType connect(ATPTradeAPI* client, ATPTradeHandler* handler)
{
    ...
    prop.user = "ATP_YQ01";                                            // 网关用户名
    prop.password = "ATP@YQ888";                                    // 网关用户密码
    prop.locations = { "120.204.239.213:32001", "127.0.0.1:32495" };      // 网关主备节点的地址+端口
    ...
    prop.client_name = "atp_demo";                                  // 客户端程序名字
    prop.client_name = "V1.0.0";                                    // 客户端程序版本
...
ATPRetCodeType login(ATPTradeAPI* client)
{
    ...
    strncpy(login_msg.fund_account_id, "12000001665701", 17);             // 资金账户ID
    strncpy(login_msg.password, "8888888", 129);                       // 客户号密码
    login_msg.login_mode = ATPCustLoginModeType::kFundAccountIDMode;   // 登录模式，资金账号登录
    login_msg.client_seq_id = g_client_seq_id++;                // 客户系统消息号
    login_msg.order_way = '1';                                  // 委托方式，自助委托
...
ATPRetCodeType send(ATPTradeAPI* client)
{
    ...
    strncpy(p->cust_id, "120000016657", 17);                    // 客户号ID
    strncpy(p->fund_account_id, "12000001665701", 17);             // 资金账户ID
    strncpy(p->account_id, "B880706370", 13);                   // 账户ID
    p->side = ATPSideConst::kBuy;                           // 买卖方向，买
    p->order_type = ATPOrdTypeConst::kFixedNew;             // 订单类型，限价
    p->price = 210000;                                      // 委托价格 N13(4)，10.0000元
    p->order_qty = 100000;                                   // 申报数量（股票为股、基金为份、上海债券为手，其他为张）N15(2)，100.00股
    p->client_seq_id = g_client_seq_id++;                   // 用户系统消息序号
    p->order_way = '1';                                     // 委托方式，自助委托
