...
{
    ATPAccountIDType account_id; ///证券账户ID（必填）
    ATPFundAccountIDType fund_account_id; ///资金账户ID（必填）
    ATPBranchIDType branch_id; ///营业部ID（必填）
...
{
    ATPCustIDType cust_id; ///客户号ID（必填）
    ATPFundAccountIDType fund_account_id; ///资金账户ID（必填）
    ATPBranchIDType branch_id; ///营业部ID（非必填）
    ATPAccountIDType account_id; ///证券账户ID（必填）
    ATPClientSeqIDType client_seq_id; ///用户系统消息序号（必填）
    ATPOrderWayType order_way; ///委托方式（是否必填以及字典项请咨询券商）
    ATPUserInfoType user_info; ///用户私有信息（非必填）
    ATPPasswordType password; ///客户密码（非必填）
    ATPExtraData extra_data; ///扩展字段（非必填）
    ATPClientFeatureCodeType client_feature_code; ///终端识别码（该字段是否必填请咨询券商）
    ATPSecurityIDType security_id; ///证券代码（必填）
    ATPMarketIDType market_id; ///市场代码（必填）
    ATPSideType side; ///买卖方向（必填）
    ATPQtyType order_qty; ///申报数量N15(2)（必填）（股票为股、基金为份、上海债券默认为张（使用时请务必与券商确认），其他为张）
    ATPPriceType price; ///委托价格N13(4)（必填）（主板市价委托填0，科创板股票市价委托必须输入保护价）
    ATPEnforceFlagType enforce_flag; ///风险强制标识（已弃用）
    ATPPriceType stop_px; ///止损价N13(4)（非必填）
    ATPOrdTypeType order_type; ///订单类型（必填）
...
struct TRADE_API APIBatchCashAuctionOrderUnit
{
    ...
    ATPMarketIDType market_id; ///市场代码（必填）
    ATPSideType side; ///买卖方向（必填）
    ATPQtyType order_qty; ///申报数量N15(2)（张）（必填）
    ATPPriceType price; ///委托价格N13(4)（必填）（主板市价委托填0，科创板股票市价委托必须输入保护价）
    ATPOrdTypeType order_type; ///订单类型（必填）
...
struct TRADE_API ATPReqBatchCashAuctionOrderMsg
{
    ...
    ATPPriceType price; ///委托价格N13(4)（必填）（主板市价委托填0，科创板股票市价委托必须输入保护价）
    ATPEnforceFlagType enforce_flag; ///风险强制标识（已弃用）
    ATPPriceType stop_px; ///止损价（必填）
    ATPOrdTypeType order_type; ///订单类型（必填）
    ATPQtyType min_qty; ///最低成交数量N15(2)（已弃用）
    ATPMaxPriceLevelsType max_price_levels; ///最多成交价位数（已弃用）
    ATPTimeInForceType time_in_force; ///订单有效时间类型（已弃用）
    ATPBatchType batch_type; ///批量委托类型（必填）
    ATPQtyType decrease_qty; ///递减数量（非必填）
    ATPQtyType max_order_qty; ///单笔上限（非必填）
    ATPQtyType min_order_qty; ///单笔下限（已弃用）
    std::vector<APIBatchCashAuctionOrderUnit> order_array; ///多证券委托组合（非必填）
...
struct TRADE_API ATPReqCreditOrderMsg
{
    ...
    ATPSecurityIDType security_id; ///证券代码（必填）
    ATPMarketIDType market_id; ///市场代码（必填）
    ATPSideType side; ///买卖方向（必填）‘S’=  融券卖出、‘B’= 融资买入
    ATPQtyType order_qty; ///申报数量N15(2)（必填）（股票为股、基金为份、上海债券默认为张（使用时请务必与券商确认），其他为张）
    ATPPriceType price; ///委托价格N13(4)（必填）（主板市价委托填0，科创板股票市价委托必须输入保护价）
    ATPEnforceFlagType enforce_flag; ///风险强制标识（已弃用）
    ATPPriceType stop_px; ///止损价N13(4)（必填）
    ATPOrdTypeType order_type; ///订单类型（必填）
    ATPQtyType min_qty; ///最低成交数量N15(2)（已弃用）
    ATPMaxPriceLevelsType max_price_levels; ///最多成交价位数（已弃用）
    ATPTimeInForceType time_in_force; ///订单有效时间类型（已弃用）
...
{
    ATPCustIDType cust_id; ///客户号ID（必填）(投资者信用账户)
    ATPFundAccountIDType fund_account_id; ///资金账户ID（必填）(投资者信用账户)
    ATPBranchIDType branch_id; ///营业部ID（必填）(投资者信用账户)
    ATPAccountIDType account_id; ///证券账户ID（必填）(投资者信用账户)
    ATPClientSeqIDType client_seq_id; ///用户系统消息序号（必填）
    ATPOrderWayType order_way; ///委托方式（是否必填以及字典项请咨询券商）
    ATPUserInfoType user_info; ///用户私有信息（非必填）
    ATPPasswordType password; ///客户密码（非必填）
    ...
    ATPSecurityIDType security_id; ///证券代码（必填）
    ATPMarketIDType market_id; ///市场代码（必填）
    ATPSideType side; ///买卖方向（必填）‘L’= 担保品返还、‘T’= 直接还券、‘V’= 担保品提交
    ATPQtyType order_qty; ///申报数量N15(2)（必填）（股票为股、基金为份、上海债券默认为张（使用时请务必与券商确认），其他为张）
    ATPEnforceFlagType enforce_flag; ///风险强制标识（已弃用）
    ATPPbuIDType dest_pbu_id; ///对方申报交易单元（必填）（买卖方向 = ‘L’、‘V’、‘$’、‘*’时：深市必传，沪市传空）
    ATPAccountIDType dest_account_id; ///对方证券账户（必填）（买卖方向 = ‘L’、‘V’、‘$’、‘*’时：深市必传，沪市传空）
    ATPRedeliverType redeliver_type; ///直接还券类型
...
struct TRADE_API ATPReqMarginRepayOrderMsg
{
    ...
    ATPSecurityIDType security_id; ///证券代码（必填）
    ATPMarketIDType market_id; ///市场代码（必填）
    ATPSideType side; ///买卖方向（必填）‘N’= 卖券还款、‘U’= 买券还券、‘W’= 融资平仓、‘X’= 融券平仓
    ATPQtyType order_qty; ///申报数量N15(2)（必填）（股票为股、基金为份、上海债券默认为张（使用时请务必与券商确认），其他为张）
    ATPPriceType price; ///委托价格N13(4)（必填）（主板市价委托填0，科创板股票市价委托必须输入保护价）
    ATPEnforceFlagType enforce_flag; ///风险强制标识（已弃用）
    ATPPriceType stop_px; ///止损价N13(4)（必填）
...
struct TRADE_API ATPReqRepoAuctionOrderMsg
{
    ...
    ATPSecurityIDType security_id; ///证券代码（必填）
    ATPMarketIDType market_id; ///市场代码（必填）
    ATPSideType side; ///买卖方向（必填）。'3'=正回购，'4'=逆回购。
    ATPQtyType order_qty; ///申报数量N15(2)（必填）（股票为股、基金为份、上海债券默认为张（使用时请务必与券商确认），其他为张）
    ATPPriceType price; ///委托价格N13(4)（必填）（市价委托填0）
    ATPEnforceFlagType enforce_flag; ///风险强制标识（已弃用）
...
struct TRADE_API ATPReqBondDistributionOrderMsg
{
    ...
    ATPSecurityIDType security_id; ///证券代码（必填）
    ATPMarketIDType market_id; ///市场代码（必填）
    ATPSideType side; ///买卖方向（必填）'1'=认购  '2'=分销
    ATPQtyType order_qty; ///申报数量N15(2)（必填）（股票为股、基金为份、上海债券默认为张（使用时请务必与券商确认），其他为张）
    ATPPriceType price; ///委托价格N13(4)（必填）（市价委托填0）
    ATPEnforceFlagType enforce_flag; ///风险强制标识（已弃用）
...
struct TRADE_API ATPReqOptionAuctionOrderMsg
{
    ...
    ATPExtraData extra_data; ///扩展字段（非必填）
    ATPClientFeatureCodeType client_feature_code; ///终端识别码（该字段是否必填请咨询券商）
    ATPSecurityIDType security_id; ///证券代码(合约编码)（必填）
    ATPMarketIDType market_id; ///市场代码（必填）
    ATPSideType side; ///买卖方向（必填）
    ATPQtyType order_qty; ///申报数量N15(2)（张）（必填）
    ATPPriceType price; ///委托价格N13(4)（必填）（市价委托填0）
    ...
    ATPMaxPriceLevelsType max_price_levels; ///最多成交价位数（已弃用）
    ATPTimeInForceType time_in_force; ///订单有效时间类型（已弃用）
    ATPPositionEffectType position_effect; ///开平仓标志（必填）
    ATPCoveredOrUncoveredType covered_or_uncovered; ///备兑标签（必填）
    ATPAccountSubCodeType account_sub_code; ///账户子编码（已弃用）
    ATPSecondaryOrderIDType secondary_order_id; ///组合编码（必填）
...
struct TRADE_API ATPReqOptionQuoteOrderMsg
{
    ...
    ATPSecurityIDType security_id; ///证券代码(合约编码)（必填）
    ATPMarketIDType market_id; ///市场代码（必填）
    ATPQtyType bid_order_qty; ///买申报数量（股票为股、基金为份、上海债券默认为张（使用时请务必与券商确认），其他为张）N15(2)（必填）
    ATPPriceType bid_price; ///买委托价格N13(4)（必填）填0时，即可进行买委托的撤单
    ATPEnforceFlagType enforce_flag; ///风险强制标识（已弃用）
    ATPClientFeatureCodeType client_feature_code; ///终端识别码（该字段是否必填请咨询券商）
    ATPQuoteReqIDType quote_req_id; ///报价请求序号（必填）
    ATPPriceType offer_price; ///卖报价N13(4)（必填）
    ATPQtyType offer_order_qty; ///卖数量（股票为股、基金为份、上海债券默认为张（使用时请务必与券商确认），其他为张）N15(2) 填0时，即可进行卖委托的撤单
    ATPPositionEffectType bid_position_effect; ///买平仓标志（必填）
    ATPPositionEffectType offer_position_effect; ///卖平仓标志（必填）
    ATPAccountSubCodeType account_sub_code; ///账户子编码（已弃用）
...
struct TRADE_API ATPReqOptionAuctionForceCloseOrderMsg
{
    ...
    ATPPositionEffectType position_effect; ///开平仓标志（必填）
    ATPAccountSubCodeType account_sub_code; ///账户子编码（已弃用）
    ATPBatchNoType batch_no; ///强平批次（必填）
...
{
    ATPSecurityIDType leg_security_id; ///成份合约代码（必填）
    ATPMarketIDType market_id; ///市场代码（必填）
    ATPLegSideType leg_side; ///持仓类型（必填）
    ATPCoveredOrUncoveredType leg_covered_or_uncovered; ///腿备兑标签（必填）
...
struct TRADE_API ATPReqOptionCombinationOrderMsg
{
    ...
    ATPSecurityIDType security_id; ///证券代码(合约编码)（必填）
    ATPMarketIDType market_id; ///市场代码（必填）
    ATPSideType side; ///买卖方向（必填）'1':解除，'2'：组合
    ATPQtyType order_qty; ///申报数量N15(2)（必填）（股票为股、基金为份、上海债券默认为张（使用时请务必与券商确认），其他为张）
    ATPEnforceFlagType enforce_flag; ///风险强制标识（已弃用）
    ATPSecurityIDType strategy_id; ///策略编码（必填）
...
{
    ATPSecurityIDType leg_security_id; ///成份合约信息（必填）
...
struct TRADE_API ATPReqETFRedemptionOrderMsg
{
    ...
    ATPExtraData extra_data; ///扩展字段（非必填）
    ATPClientFeatureCodeType client_feature_code; ///终端识别码（该字段是否必填请咨询券商）
    ATPSecurityIDType security_id; ///基金证券代码（必填）
    ATPMarketIDType market_id; ///市场代码（必填）
    ATPSideType side; ///买卖方向（必填）'D'=申购  'E'=赎回  'P'=实物申购  'R'=实物赎回
    ATPQtyType order_qty; ///申赎基金份额N15(2)（必填）（股票为股、基金为份、上海债券默认为张（使用时请务必与券商确认），其他为张）
    ATPEnforceFlagType enforce_flag; ///风险强制标识（已弃用）
...
struct TRADE_API ATPReqIssueOrderMsg
{
    ...
    ATPSecurityIDType security_id; ///证券代码（必填）
    ATPMarketIDType market_id; ///市场代码（必填）
    ATPQtyType order_qty; ///申报数量N15(2)（股）（必填）
    ATPPriceType price; ///委托价格N13(4)（元）（必填）
    ATPEnforceFlagType enforce_flag; ///风险强制标识（已弃用）
...
struct TRADE_API ATPReqSwapPutbackOrderMsg
{
    ...
    ATPSecurityIDType security_id; ///证券代码（必填）
    ATPMarketIDType market_id; ///市场代码（必填）
    ATPSideType side; ///买卖方向（必填）'5'=转股  '6'=回售 '9'=回售撤销
    ATPQtyType order_qty; ///申报数量N15(2)（必填）（股票为股、基金为份、上海债券默认为张（使用时请务必与券商确认），其他为张）
    ATPPriceType price; ///委托价格N13(4)（必填）（市价委托填0）
    ATPEnforceFlagType enforce_flag; ///风险强制标识（已弃用）
    ATPShareNegotiabilityType share_negotiability; ///股份性质
...
struct TRADE_API ATPReqDesignationOrderMsg
{
    ...
    ATPDesignationTransTypeType designation_trans_type; ///转托管注册类型（必填）
    ATPPBUIDType transferee_pbu_id; ///转入托管单元代码（必填）
...
struct TRADE_API ATPReqDesignatedTransactionOrderMsg
{
    ...
    ATPExtraData extra_data; ///扩展字段（非必填）
    ATPClientFeatureCodeType client_feature_code; ///终端识别码（该字段是否必填请咨询券商）
    ATPSecurityIDType security_id; ///证券代码（必填）"799998"=撤销指定交易  "799999"=登记指定交易
    ATPEnforceFlagType enforce_flag; ///风险强制标识（已弃用）
    ATPPBUIDType designated_pbu_id; ///指定的交易单元（必填）
...
struct TRADE_API ATPReqLOFOrderMsg
{
    ...
    ATPSecurityIDType security_id; ///基金证券代码（必填）
    ATPMarketIDType market_id; ///市场代码（必填）
    ATPSideType side; ///买卖方向（必填）'D'=申购  'E'=赎回
    ATPQtyType order_qty; ///赎回基金份额N15(2)（必填）（股票为股、基金为份、上海债券默认为张（使用时请务必与券商确认），其他为张）
    ATPEnforceFlagType enforce_flag; ///风险强制标识（已弃用）
    ATPAmtType cash_order_qty; ///申购金额N18(4)（必填）
...
struct TRADE_API ATPReqTenderOfferOrderMsg
{
    ...
    ATPSecurityIDType security_id; ///证券代码（必填）
    ATPMarketIDType market_id; ///市场代码（必填）
    ATPSideType side; ///买卖方向（必填）'7'=预受要约  '8'= 解除预受要约
    ATPQtyType order_qty; ///申报数量N15(2)（必填）（股票为股、基金为份、上海债券默认为张（使用时请务必与券商确认），其他为张）
    ATPEnforceFlagType enforce_flag; ///风险强制标识（已弃用）
    ATPTendererType tenderer; ///要约收购参与人编码（必填）
...
struct TRADE_API ATPReqNetVotingOrderMsg
{
    ...
    ATPSecurityIDType security_id; ///证券代码（必填）
    ATPMarketIDType market_id; ///市场代码（必填）
    ATPQtyType order_qty; ///申报数量N15(2)（必填）累积投票议案时，OrderQty字段填写投票数量，其他情况填0
    ATPEnforceFlagType enforce_flag; ///风险强制标识（已弃用）
    ATPGeneralMeetingSeqType general_meeting_seq; ///股东大会编码（必填）
    ATPVotingProposalType voting_proposal; ///投票议案编号及子议案编号（沪深通用），议案和子议案用小数点分隔。例如：2.01，表示议案编号2，子议案编号1（必填）
    ATPVotingPreferenceType voting_preference; ///投票意向（当为非累积投票议案时）（必填）
...
struct TRADE_API ATPReqPasswordServiceOrderMsg
{
    ...
    ATPSecurityIDType security_id; ///证券代码（必填）
    ATPMarketIDType market_id; ///市场代码（必填）
    ATPSideType side; ///买卖方向（必填）。 'A'= Activation 密码激活 ，'C'= Cancellation 密码注销
    ATPQtyType order_qty; ///申报数量N15(2)（必填）（股票为股、基金为份、上海债券默认为张（使用时请务必与券商确认），其他为张）
    ATPEnforceFlagType enforce_flag; ///风险强制标识（已弃用）
    ATPValidationCodeType validation_code; ///密码(可变长字段)激活校验码（必填）
...
struct TRADE_API ATPReqWarrantExerciseOrderMsg
{
    ...
    ATPClientFeatureCodeType client_feature_code; ///终端识别码（该字段是否必填请咨询券商）
    ATPSecurityIDType security_id; ///证券代码（必填）
    ATPMarketIDType market_id; ///市场代码（必填），固定填深圳市场代码
    ATPQtyType order_qty; ///申报数量N15(2)（必填）（股票为股、基金为份、上海债券默认为张（使用时请务必与券商确认），其他为张）
    ATPPriceType price; ///委托价格N13(4)（必填）
    ATPEnforceFlagType enforce_flag; ///风险强制标识（已弃用）
...
struct TRADE_API ATPReqStructuredFundOrderMsg
{
    ...
    ATPSecurityIDType security_id; ///证券代码（必填）
    ATPMarketIDType market_id; ///市场代码（必填），固定填深圳市场代码
    ATPSideType side; ///买卖方向（必填）'O'=Open 分拆 , 'M'=Merge 合并
    ATPQtyType order_qty; ///申报数量N15(2)（必填）（股票为股、基金为份、上海债券默认为张（使用时请务必与券商确认），其他为张）
    ATPEnforceFlagType enforce_flag; ///风险强制标识（已弃用）
...
struct TRADE_API ATPReqCoveredLockOrderMsg
{
    ...
    ATPClientFeatureCodeType client_feature_code; ///终端识别码（该字段是否必填请咨询券商）
    ATPSecurityIDType security_id; ///证券代码（必填）
    ATPSideType side; ///买卖方向（必填）'Z'=锁定  'Y'= 解锁
    ATPQtyType order_qty; ///申报数量N15(2)（必填）（股票为股、基金为份、上海债券默认为张（使用时请务必与券商确认），其他为张）
    ATPEnforceFlagType enforce_flag; ///风险强制标识（已弃用）
...
struct TRADE_API ATPReqTibAfterHourAuctionOrderMsg
{
    ...
    ATPClientFeatureCodeType client_feature_code; ///终端识别码（该字段是否必填请咨询券商）
    ATPSecurityIDType security_id; ///证券代码（必填）
    ATPSideType side; ///买卖方向（必填）'1'=买 ，'2'=卖
    ATPQtyType order_qty; ///申报数量N15(2)（必填）（股票为股、基金为份、上海债券默认为张（使用时请务必与券商确认），其他为张）
    ATPPriceType price; ///委托价格N13(4)（必填）
    ATPEnforceFlagType enforce_flag; ///风险强制标识（已弃用）
    ATPMarketIDType market_id; ///市场代码（必填）
...
struct TRADE_API ATPReqTurnToDisposalOrderMsg
{
    ...
    ATPPriceType price; ///委托价格N13(4)（必填）（市价委托填0）
    ATPEnforceFlagType enforce_flag; ///风险强制标识（已弃用）
    ATPPBUIDType disposal_pbu; ///划入待处置券的交易单元（必填）
    ATPAccountIDType disposal_account_id; ///划入待处置券的证券账户（必填）
...
struct TRADE_API ATPReqPledgeInOrOutOrderMsg
{
    ...
    ATPSecurityIDType security_id; ///证券代码（必填）
    ATPMarketIDType market_id; ///市场代码（必填）
    ATPSideType side; ///买卖方向（必填）‘H’表示入库‘I’表示出库
    ATPQtyType order_qty; ///申报数量N15(2)（必填）（股票为股、基金为份、上海债券默认为张（使用时请务必与券商确认），其他为张）
    ATPEnforceFlagType enforce_flag; ///风险强制标识（已弃用）
...
struct TRADE_API ATPReqCancelOrderMsg
{
    ...
    ATPExtraData extra_data; ///扩展字段（非必填）
    ATPClientFeatureCodeType client_feature_code; ///终端识别码（该字段是否必填请咨询券商）
    ATPMarketIDType market_id; ///市场代码（非必填）
    ATPClOrdNOType orig_cl_ord_no; ///原始交易客户方（券商）订单编号，指示被撤消订单的ClOrdNO。（必填）
...
{
    ATPCustIDType cust_id; ///客户号ID（必填），如果证券账户为全空格，则按客户号进行撤单；如果均为空格，则指令无效
    ATPFundAccountIDType fund_account_id; ///资金账户ID（必填）
    ATPAccountIDType account_id; ///证券账户ID（必填），填空则按照客户号一键撤单，填写了证券账户，则优先按证券账户进行撤单
    ATPClientSeqIDType client_seq_id; ///用户系统消息序号（必填）
    ATPOrderWayType order_way; ///委托方式（是否必填以及字典项请咨询券商）
    ATPUserInfoType user_info; ///用户私有信息（非必填）
    ATPPasswordType password; ///客户密码（非必填）
    ATPExtraData extra_data; ///扩展字段（非必填）
    ATPClientFeatureCodeType client_feature_code; ///终端识别码（该字段是否必填请咨询券商）
    ATPBusinessTypeType business_type; ///业务类型（必填），撤所有业务类型时，填0
    ATPSecurityIDType security_id; ///证券代码（必填），全填空格表示撤销所有
    ATPMarketIDType market_id; ///市场代码（必填）
    ATPBranchIDType branch_id; ///营业部ID（必填）
    BusinessAbstractType business_abstract_type; ///抽象业务编码（必填）
...
struct TRADE_API ATPRspOrderStatusAckMsg
{
    ...
    ATPClOrdNOType orig_cl_ord_no; ///对于撤单返回的状态响应，为原始委托的客户合同号，指示被撤消订单的ClOrdNO; 对于委托确认，取值为0。
...
struct TRADE_API ATPRspBatchOrderStatusAckMsg
{
    ...
    ATPExtraData extra_data; ///扩展字段
    ATPClientSeqIDType client_seq_id; ///用户系统消息序号
    ATPClOrdNOType orig_parent_cl_ord_no; ///原始委托的客户合同号，指示被撤消批量订单的ClOrdNO;
    ATPRejectReasonCodeType reject_reason_code; ///拒绝原因代码；可调用GetErrorInfo错误码查询函数，传入错误码及错误码类型-5 拒绝原因错误码类型，即可查询到错误描述信息
    ATPOrdRejReasonType ord_rej_reason; ///拒绝原因描述信息；当交易所拒绝时，这里填写交易所返回的拒绝原因代码。可调用GetErrorInfo错误码查询函数，传入错误码及错误码类型-1交易所拒绝原因错误码类型，即可查询错误描述信息
    ATPOrdTypeType order_type; ///订单类型
    ATPTimeInForceType time_in_force; ///订单有效时间类型（无意义）
...
{
    ATPSecurityIDType security_id; ///成分股证券代码，或资金代码（仅上交所才资金代码）
    ATPMarketIDType market_id; ///市场代码
    ATPQtyType qty; ///成交数量或资金比例因子N15(2)（股票为股、基金为份、上海债券默认为张（使用时请务必与券商确认），其他为张）
    ATPAmtType amt; ///成交金额N18(4)
    ATPPriceType price; ///成交价格N13(4)
...
struct TRADE_API ATPRspETFRedemptionTradeERMsg
{
    ...
    ATPSideType side; ///买卖方向 'D'=回购  'E'=赎回
    ATPTimeStampType transact_time; ///回报时间
    ATPUserInfoType user_info; ///用户私有信息
    ATPOrderIDType order_id; ///交易所订单编号
    ATPClOrdIDType cl_ord_id; ///申报合同号 上交所：以QP1开头，表示为交易所保证金强制平仓；以CV1开头，表示为交易所备兑强制平仓；
...
struct TRADE_API ATPReqOrderQueryMsg
{
    ATPCustIDType cust_id; ///客户号ID（必填）
    ATPFundAccountIDType fund_account_id; ///资金账户ID（必填）
    ATPAccountIDType account_id; ///证券账户ID（非必填）（一户两地账户此字段必填）
    ATPClientSeqIDType client_seq_id; ///用户系统消息序号（必填）
    ATPUserInfoType user_info; ///用户私有信息（非必填）
    ATPPasswordType password; ///客户密码（非必填）
    ATPExtraData extra_data; ///扩展字段（非必填）
    ATPClientFeatureCodeType client_feature_code; ///终端识别码（该字段是否必填请咨询券商）
    ATPClOrdNOType cl_ord_no; ///客户订单编号（非必填）（默认值为0，表示查所有，一户两地账户表示查单个证券账户下所有）
    ATPMarketIDType market_id; ///市场代码（非必填）（默认值为0，表示查所有，一户两地账户表示查单个证券账户下所有）
    ATPSecurityIDType security_id; ///证券代码（非必填）（默认值为空字符串，表示查所有，一户两地账户表示查单个证券账户下所有）
    ATPBusinessTypeType business_type; ///业务类型（非必填）（默认值为0，表示查所有，一户两地账户表示查单个证券账户下所有）
    ATPSideType side; ///买卖方向（非必填）（默认值为'0'，表示查所有，一户两地账户表示查单个证券账户下所有）
    ATPOrderQueryConditionType order_query_condition; ///委托查询条件（非必填）（默认值为0，表示查所有，一户两地账户表示查单个证券账户下所有）
    ATPReportIndexType query_index; ///起始查询索引号（非必填）（默认值为0，表示从头开始查）
    ATPTotalNumType return_num; ///查询返回数量（非必填）（默认值为0，表示按能返回的最大数量返回，具体数量请咨询券商）
    ATPReturnSeqType return_seq; ///返回顺序，按时间排序（非必填）（默认值为1，表示按时间顺序排序）
    BusinessAbstractType business_abstract_type; ///抽象业务编码（必填）
...
struct TRADE_API ATPReqBatchOrderQueryMsg
{
    ...
    ATPExtraData extra_data; ///扩展字段（非必填）
    ATPClientFeatureCodeType client_feature_code; ///终端识别码（该字段是否必填请咨询券商）
    ATPClOrdNOType parent_cl_ord_no; ///客户订单编号（必填）
    ATPReportIndexType query_index; ///起始查询索引号（非必填）（默认值为0，表示从头开始查）
    ATPTotalNumType return_num; ///查询返回数量（非必填）（默认值为0，表示按能返回的最大数量返回，具体数量请咨询券商）
...
struct TRADE_API ATPReqCombinationQueryMsg
{
    ...
    ATPClientFeatureCodeType client_feature_code; ///终端识别码（该字段是否必填请咨询券商）
    ATPMarketIDType market_id; ///市场代码（非必填）（默认值为0，表示查所有，一户两地账户表示查单个证券账户下所有）
    ATPSecondaryOrderIDType secondary_order_id; ///组合编码（非必填）（默认值为空字符串，表示查所有，一户两地账户表示查单个证券账户下所有）
    ATPReportIndexType query_index; ///起始查询索引号（非必填）（默认值为0，表示从头开始查）
    ATPTotalNumType return_num; ///查询返回数量（非必填）（默认值为0，表示按能返回的最大数量返回，具体数量请咨询券商）
...
struct TRADE_API ATPReqExtQueryContractSpecificationsMsg
{
    ...
    ATPClientFeatureCodeType client_feature_code; ///终端识别码（该字段是否必填请咨询券商）
    ATPBranchIDType branch_id; ///营业部ID（必填）
    ATPCloseOutFlagType close_out_flag; ///了结标识（非必填）（默认值为'0'，表示查所有，一户两地账户表示查单个证券账户下所有）
    ATPCashMarginSetType cash_margin_set; ///信用标识集合（非必填）（默认值为空字符串）（标志位之间用逗号隔开）（"1,3,7"=融资 ，"2,4,5,6,7"=融券）
    ATPDate8Type opening_date; ///开仓日期（非必填）（默认值为0）
    ATPOpeningSerialNumType opening_serial_num; ///开仓流水号（非必填）（默认值为空字符串）
    ATPReportIndexType query_index; ///起始查询索引号（非必填）（默认值为0，表示从头开始查）
    ATPTotalNumType return_num; ///查询返回数量（非必填）（默认值为0，表示按能返回的最大数量返回，具体数量请咨询券商）
    ATPReturnSeqType return_seq; ///返回顺序，按时间排序（非必填）（默认值为1，表示按时间顺序排序）
...
struct TRADE_API ATPReqExtQueryMarginUseableMsg
{
    ...
    ATPClientFeatureCodeType client_feature_code; ///终端识别码（该字段是否必填请咨询券商）
    ATPBranchIDType branch_id; ///营业部ID（必填）
    ATPQueryContentsType query_contents; ///查询内容（必填）'0'= 只查询余额，'1'= 查询余额及明细
...
struct TRADE_API ATPReqExtQueryFundLiabilityMsg
{
    ...
    ATPClientFeatureCodeType client_feature_code; ///终端识别码（该字段是否必填请咨询券商）
    ATPBranchIDType branch_id; ///营业部ID（必填）
    ATPCurrencyType currency; ///货币种类（必填）（仅支持人民币）
...
struct TRADE_API ATPReqExtQueryShareLiabilityMsg
{
    ...
    ATPClientFeatureCodeType client_feature_code; ///终端识别码（该字段是否必填请咨询券商）
    ATPBranchIDType branch_id; ///营业部ID（必填）
    std::vector<APIAccountIDUnit> account_id_array; ///待查证券账户组（必填）
    ATPMarketIDType market_id; ///市场代码（非必填）（默认值为0，表示查所有，一户两地账户表示查单个证券账户下所有）
    ATPSecurityIDType security_id; ///证券代码（非必填）（默认值为空字符串，表示查所有，一户两地账户表示查单个证券账户下所有）
    ATPReportIndexType query_index; ///起始查询索引号（非必填）（默认值为0，表示从头开始查）
    ATPTotalNumType return_num; ///查询返回数量（非必填）（默认值为0，表示按能返回的最大数量返回，具体数量请咨询券商）
...
struct TRADE_API ATPReqExtQueryContractSpecificationsManagerMsg
{
    ...
    ATPCloseOutFlagType close_out_flag; ///了结标识（非必填）（默认值为'0'，表示查所有，一户两地账户表示查单个证券账户下所有）
    ATPCashMarginSetType cash_margin_set; ///信用标识集合（非必填）（默认值为空字符串）（标志位之间用逗号隔开）（"1,3 ,7"=融资 ，"2, 4, 5, 6, 7"=融券）
    ATPDate8Type opening_date; ///开仓日期（非必填）(默认填为0）
    ATPOpeningSerialNumType opening_serial_num; ///开仓流水号（非必填）（默认值为空字符串）
    ATPReportIndexType query_index; ///起始查询索引号（非必填）（默认值为0，表示从头开始查）
    ATPTotalNumType return_num; ///查询返回数量（非必填）（默认值为0，表示按能返回的最大数量返回，具体数量请咨询券商）
...
struct TRADE_API ATPReqExtQueryMarginUnderlyingMsg
{
    ...
    ATPMarketIDType market_id; ///市场代码（非必填）（默认值为0，表示查所有，一户两地账户表示查单个证券账户下所有）
    ATPSecurityIDType security_id; ///证券代码（非必填）（默认值为空字符串，表示查所有，一户两地账户表示查单个证券账户下所有）
    ATPCRDUnderlyingTypeType underlying_query_type; ///标的类型（非必填）（默认值为0，表示查所有，一户两地账户表示查单个证券账户下所有）
    ATPReportIndexType query_index; ///起始查询索引号（非必填）（默认值为0，表示从头开始查）
    ATPTotalNumType return_num; ///查询返回数量（非必填）（默认值为0，表示按能返回的最大数量返回，具体数量请咨询券商）
...
{
    ATPCustIDType cust_id; ///客户号ID（必填）预留
    ATPFundAccountIDType fund_account_id; ///资金账户ID（必填）
    ATPAccountIDType account_id; ///证券账户ID（必填）
    ATPClientSeqIDType client_seq_id; ///用户系统消息序号（必填）
    ATPUserInfoType user_info; ///用户私有信息（非必填）
...
struct TRADE_API ATPReqFundQueryMsg
{
    ...
    ATPExtraData extra_data; ///扩展字段（非必填）
    ATPClientFeatureCodeType client_feature_code; ///终端识别码（该字段是否必填请咨询券商）
    ATPCurrencyType currency; ///货币种类（必填）
...
struct TRADE_API ATPReqShareQueryMsg
{
    ...
    std::vector<APIAccountIDUnit> account_id_array; ///待查证券账户组（必填）
    ATPBusinessTypeType business_type; ///业务类型（非必填）（默认值为0，表示查所有，一户两地账户表示查单个证券账户下所有）
    ATPMarketIDType market_id; ///市场代码（必填）（默认值为0，表示查所有，一户两地账户表示查单个证券账户下所有）
    ATPSecurityIDType security_id; ///证券代码（非必填）（默认值为空字符串，表示查所有，一户两地账户表示查单个证券账户下所有）
    ATPReportIndexType query_index; ///起始查询索引号（非必填）（默认值为0，表示从头开始查）
    ATPTotalNumType return_num; ///查询返回数量（非必填）（默认值为0，表示按能返回的最大数量返回，具体数量请咨询券商）
...
struct TRADE_API ATPReqAccountContractFundQueryMsg
{
    ATPCustIDType cust_id; ///客户号ID（必填）
    ATPFundAccountIDType fund_account_id; ///资金账户ID（必填）
    ATPAccountIDType account_id; ///证券账户ID（非必填）
    ATPClientSeqIDType client_seq_id; ///用户系统消息序号（必填）
    ATPClientFeatureCodeType client_feature_code; ///终端识别码（该字段是否必填请咨询券商）
    ATPUserInfoType user_info; ///用户私有信息（非必填）
    ATPPasswordType password; ///客户密码（非必填）
    ATPBranchIDType branch_id; ///营业部（必填）
    ATPExtraData extra_data; ///扩展字段（非必填）
...
struct TRADE_API ATPReqTradeCollectQueryMsg
{
    ...
    ATPExtraData extra_data; ///扩展字段（非必填）
    ATPClientFeatureCodeType client_feature_code; ///终端识别码（该字段是否必填请咨询券商）
    ATPMarketIDType market_id; ///市场代码（必填）,填0查所有
    ATPSecurityIDType security_id; ///证券代码（必填）
    ATPBusinessTypeType business_type; ///业务类型（必填）,必填。填0查所有
    BusinessAbstractType business_abstract_type; ///抽象业务编码（必填）
...
struct TRADE_API ATPReqTIBIPOQtyQueryMsg
{
    ...
    ATPExtraData extra_data; ///扩展字段（非必填）
    ATPClientFeatureCodeType client_feature_code; ///终端识别码（该字段是否必填请咨询券商）
    ATPMarketIDType market_id; ///市场代码（必填，仅支持101-上海市场）
...
struct TRADE_API ATPReqCoverMaxOrderQtyQueryMsg
{
    ...
    ATPMarketIDType market_id; ///市场代码（必填）
    ATPSecurityIDType security_id; ///证券代码（必填）
    ATPBusinessTypeType business_type; ///业务类型（必填）
    ATPSideType side; ///买卖方向（必填）
    ATPPriceType price; ///委托价格N13(4)（必填）
    ATPAccountSubCodeType account_sub_code; ///账户子编码（已弃用）
    ATPPositionEffectType position_effect; ///开平仓标志（必填）
...
struct TRADE_API ATPReqHKExtQueryMaxQueryOrderQtyMsg
{
    ...
    ATPBusinessTypeType business_type; ///业务类型（必填）
    ATPSideType side; ///买卖方向（必填）
    ATPPriceType price; ///委托价格 N13(4)（必填）
    ATPOrderWayType order_way; ///委托方式（是否必填以及字典项请咨询券商）
    ATPEntitlementType entitlement_number; ///权益编码（非必填）
...
struct TRADE_API ATPReqMaxOrderQtyQueryMsg
{
    ...
    ATPSideType side; ///买卖方向（必填）
    ATPPriceType price; ///委托价格N13(4)（必填）
    ATPImmediateEffectType immediate_effect; ///实时划转标志（必填）
    ATPRedeliverType redeliver_type; ///直接还券类型（必填）
...
struct TRADE_API ATPReqNewMaxOrderQtyQueryMsg
{
    ...
    ATPPriceType price; ///委托价格N13(4)（必填）
    ATPOrdTypeType order_type; ///订单类型（必填）
    ATPTrdSubTypeType trd_sub_type; ///是否受限（非必填）（仅适用于大宗交易）
    ATPOrderWayType order_way; ///委托方式（是否必填以及字典项请咨询券商）
...
{
    ATPCustIDType cust_id; ///客户号ID（非必填）（当登录模式为1-客户号登录模式时，该字段必填）
    ATPFundAccountIDType fund_account_id; ///资金账户ID（非必填）（当登录模式为2-资金账户登录模式时，该字段必填）
    ATPAccountIDType account_id; ///证券账户ID（非必填）（当登录模式为3-证券账户登录模式时，该字段必填）
    ATPClientSeqIDType client_seq_id; ///用户系统消息序号（必填）
    ATPUserInfoType user_info; ///用户私有信息（非必填）
    ATPExtraData extra_data; ///扩展字段（非必填）
    ATPClientFeatureCodeType client_feature_code; ///终端识别码（该字段是否必填请咨询券商）
    ATPPasswordType password; ///客户密码（必填）
    ATPOrderWayType order_way; ///委托方式（是否必填以及字典项请咨询券商）
    ATPBranchIDType branch_id; ///营业部ID（非必填）（当登录模式为2-资金账户登录模式时，该字段是否必填请咨询券商）
    ATPLoginModeType login_mode; ///客户号登录模式（必填）
...
{
    ATPCustIDType cust_id; ///客户号ID（非必填）
    ATPFundAccountIDType fund_account_id; ///资金账户ID（必填）
    ATPAccountIDType account_id; ///证券账户ID（非必填）
    ATPClientSeqIDType client_seq_id; ///用户系统消息序号（必填）
    ATPUserInfoType user_info; ///用户私有信息（非必填）
...
struct TRADE_API ATPReqExtFundTransferOtherMsg
{
    ...
    ATPExtraData extra_data; ///扩展字段（非必填）
    ATPClientFeatureCodeType client_feature_code; ///终端识别码（该字段是否必填请咨询券商）
    ATPPasswordType password; ///密码（非必填）
    ATPBoolType is_account_partition; ///账户是否按证券账户分区（已弃用）
    ATPCurrencyType currency; ///货币种类（非必填，默认值 "CNY"）
    ATPTransferWayType transfer_way; ///划拨方式（必填）：1=转入；2=转出
    ATPAmtType transfer_value; ///划拨金额 N18(4)（必填）
    ATPBranchIDType branch_id; ///营业部ID（必填）
    ATPRemarkType remark; ///备注（必填）
    ATPOrderWayType order_way; ///委托方式（是否必填以及字典项请咨询券商）
...
struct TRADE_API ATPReqExtRepayOtherMsg
{
    ...
    ATPPasswordType password; ///密码（非必填）
    ATPOrderWayType order_way; ///委托方式（是否必填以及字典项请咨询券商）
    ATPCurrencyType currency; ///货币种类（非必填）（默认值 "CNY"）仅支持人民币
    ATPAmtType repay_value; ///还款金额 N18(4)（必填）
    ATPMarketIDType market_id; ///市场代码（必填）
    ATPBranchIDType branch_id; ///营业部ID（必填）
    ATPRepayWayType repay_way; ///还款方式（必填）
...
struct TRADE_API APIContractSpecificationsUnit
{
    ...
    ATPQtyType opening_qty; ///开仓数量（股票为股、基金为份、上海债券默认为张（使用时请务必与券商确认），其他为张）N15(2)
    ATPPriceType opening_price; ///开仓价格N13(4)
    ATPAmtType repayment_amt; ///已偿还金额N18(4)
    ATPQtyType repayment_qty; ///已偿还数量（股票为股、基金为份、上海债券默认为张（使用时请务必与券商确认），其他为张）N15(2)
    ...
    ATPDepositRatioType deposit_ration; ///保证金比例
...
struct TRADE_API ATPRspQueryCreditAmountResultMsg
{
    ...
    std::vector<APICreditAmountUnit> order_array; ///查询结果组合(单客户查询为1)
...
struct TRADE_API ATPRspQueryFundLiabilityResultMsg
{
    ...
    ATPAmtType private_amount_occupied; ///保底额度占用
    ATPAmtType public_amount_occupied; ///公用额度占用
    ATPAmtType private_amount_frozen; ///保底额度冻结
    ATPAmtType public_amount_frozen; ///公用额度冻结
...
struct TRADE_API APIShareLiabilityUnit
{
    ...
    ATPMarketIDType market_id; ///市场代码
    ATPAccountIDType account_id; ///证券账户ID
    ATPQtyType real_time_qty; ///实时数量（股票为股、基金为份、上海债券默认为张（使用时请务必与券商确认），其他为张）N15(2)
    ATPAmtType market_value; ///市值N18(4)
    ATPQtyType init_qty; ///期初数量（股票为股、基金为份、上海债券默认为张（使用时请务必与券商确认），其他为张）N15(2)
    ATPQtyType today_open; ///当日开仓
    ATPQtyType today_repay; ///当日偿还
    ATPQtyType today_leaves; ///当日余券
    ATPAmtType margin_required; ///保证金占用N18(4)
    ATPQtyType temp_add; ///临时调增
    ATPQtyType temp_sub; ///临时调减
...
struct TRADE_API APIContractSpecificationsManagerUnit
{
    ...
    ATPAmtType margin_required; ///保证金占用N18(4)
    ATPQtyType total_liability_qty; ///总负债数量（股票为股、基金为份、上海债券默认为张（使用时请务必与券商确认），其他为张）N15(2)
...
struct TRADE_API APIMarginUnderlyingUnit
{
    ATPMarketIDType market_id; ///市场代码
    ATPSecurityIDType security_id; ///证券代码
    UnderlyingTypeType underlying_type; ///标的类型(按bit位控制，0为否1为是，自低到高分别为担保品、融券、融资)
    ATPCRDUnderlyingRateType translation_proportion; ///折算率
    ATPCRDBusinessSwitchType business_switch; ///业务开关(按bit位控制，0为否1为是，自低到高分别为暂停提交、禁止展期、集中度控制、高流通性证券)
    ATPDepositRatioType financing_guarantee_ratio; ///融资保证金比率
    ATPDepositRatioType security_guarantee_ratio; ///融券保证金比率
    ATPQtyType margin_qty; ///融券专户可用数量
...
struct TRADE_API ATPRspQueryCreditInfoResultMsg
{
    ...
    ATPExtraData extra_data; ///扩展字段
    ATPBranchIDType branch_id; ///营业部ID
    ATPContractStatusType contrac_status; ///合同状态
    ATPCreditRatingType credit_rating; ///信用级别
    ATPCloseoutStatusType closeout_status; ///平仓状态
    ATPInterestRateProductType interest_product; ///利率产品
    ATPDepositRatioType financing_guarantee_ratio; ///融资保证金比例
    ATPDepositRatioType security_guarantee_ratio; ///融券保证金比例
...
struct TRADE_API ATPRspQueryFundAssetsInfoResultMsg
{
    ...
    ATPAmtType margin_required; ///保证金占用N18(4)
    ATPMtnGuaranteeRatioType maintains_margin_ratio1; ///维持担保比例1
    ATPMtnGuaranteeRatioType maintains_margin_ratio2; ///维持担保比例2
    ATPAmtType unfilled_crd_sell_amt; ///融券卖出未成交金额
    ATPAmtType unfilled_crd_buy_amt; ///融资买入未成交金额
...
struct TRADE_API APIOrderUnit
{
    ...
    ATPClOrdNOType orig_cl_ord_no; ///对于撤单订单，为原始交易客户方（券商）订单编号，指示被撤消订单的ClOrdNO; 对于普通订单，取值为0。
    ATPAmtType frozen_trade_value; ///冻结交易金额N18(4)
    ATPAmtType frozen_fee; ///冻结费用N18(4)
    ATPCurrencyType currency; ///货币种类
    ATPRejectReasonCodeType reject_reason_code; ///拒绝原因代码；可调用GetErrorInfo错误码查询函数，传入错误码及错误码类型-5 拒绝原因错误码类型，即可查询到错误描述信息
    ATPOrdRejReasonType ord_rej_reason; ///拒绝原因描述信息；当交易所拒绝时，这里填写交易所返回的拒绝原因代码。可调用GetErrorInfo错误码查询函数，传入错误码及错误码类型-1交易所拒绝原因错误码类型，即可查询错误描述信息
    ATPAmtType order_entrusted_amt; ///委托金额
    ATPAmtType order_cum_transaction_amt; ///成交金额
...
struct TRADE_API ATPReqExtFundQueryMsg
{
    ...
    ATPUserInfoType user_info; ///用户私有信息（非必填）
    ATPPasswordType password; ///客户密码（非必填）
    ATPCurrencyType currency; ///货币种类（非必填）（默认值为空，表示查所有）
...
struct TRADE_API ATPRspExtFundQueryResultMsg
{
    ...
    std::vector<APIExtFundUnit> ext_query_fund_array; ///成交汇总查询结果组合
...
struct TRADE_API APITradeOrderUnit
{
    ...
    ATPExecTypeType exec_type; ///当前订单执行类型,若为交易所确认撤单的Exectype=4，Cancelled表示已撤销
    ATPExecIDType exec_id; ///执行编号 ,对于交易所确认撤单的委托执行编号为空
    ATPClOrdNOType cl_ord_no; ///客户订单编号,原订单的客户订单编号，若此笔记录为撤单，则应该是撤单的原订单
    ATPOrderIDType order_id; ///交易所订单编号,交易所订单编号，对于交易所确认撤单的记录，OrderID为空
    ATPClOrdIDType cl_ord_id; ///申报合同号,上交所：以QP1开头,表示为交易所保证金强制平仓；以CV1开头,表示为交易所备兑强制平仓；
    ATPTimeStampType transact_time; ///成交时间 ,成交时间，对于交易所确认撤单，取交易所确认撤单的回报时间
    ATPPriceType last_px; ///成交价格，对于交易所确认撤单，取0N13(4)
    ATPQtyType last_qty; ///成交数量 ,对于交易所确认撤单，取每笔撤单的数量（股票为股、基金为份、上海债券默认为张（使用时请务必与券商确认），其他为张）N15(2)
...
struct TRADE_API APIShareRecordETFArray
{
    ATPSecurityIDType security_id; ///证券代码
    ATPMarketIDType market_id; ///市场代码
    ATPQtyType qty; ///成交数量或资金比例因子N15(2)
    ATPAmtType amt; ///成交金额N18(4)
    ATPPriceType price; ///成交价格N13(4)
    ATPETFTradeReportTypeType etf_trade_report_type; ///ETF成交回报类型
    ATPExecIDType exec_id; ///执行编号
...
struct TRADE_API ATPRspFundQueryResultMsg
{
    ...
    ATPAmtType frozen_all; ///当前所有冻结N18(4)
    ATPPartitionType te_partition_no; ///所属分区号
    ATPAmtType credit_sell_frozen; ///融券卖出冻结金额N18(4)
...
struct TRADE_API APIShareUnit
{
    ...
    ATPQtyType leaves_qty; ///剩余股份数量（包含当日买入部分,拆分合并、申购赎回）（股票为股、基金为份、上海债券默认为张（使用时请务必与券商确认），其他为张）N15(2)
    ...
    ATPCurrencyType currency; ///货币种类
    ATPQtyType init_crd_sell_buy_share_qty; ///日初融券买入量
    ATPAmtType init_crd_sell_occupied_amt; ///日初融券占用金额
    ATPQtyType cur_crd_sell_occupied_qty; ///当前融券买入量
    ATPAmtType cur_crd_sell_occupied_amt; ///当前融券占用金额
...
struct TRADE_API APIContractUnit
{
    ...
    ATPPositionSideType position_side; ///持仓方向
...
{
    ATPPartitionType partition; ///分区号（必填）
    ATPAgwIndexType index; ///回报记录号（必填）
...
{
    ATPSeqNumType client_seq_id; ///用户系统消息序号（必填）
...
struct TRADE_API ATPReqAfterHoursPricingOrderMsg
{
    ...
    ATPPriceType price; ///委托价格N13(4)（必填）（主板市价委托填0，科创板股票市价委托必须输入保护价）
    ATPEnforceFlagType enforce_flag; ///风险强制标识（已弃用）
    ATPPricePropertyType price_property; ///价格类型（必填）
...
struct TRADE_API ATPReqBlockTradeIntentionOrderMsg
{
    ...
    ATPPriceType price; ///委托价格N13(4)（必填）
    ATPEnforceFlagType enforce_flag; ///风险强制标识（已弃用）
    ATPTrdSubTypeType trd_sub_type; ///是否受限（必填）0：非受限，1:受限(仅上海)
    ATPContactorType contactor; ///联系人（必填）
    ATPContactInfoType contact_info; ///联系人信息（必填）
...
struct TRADE_API ATPReqBlockTradeTransactionOrderMsg
{
    ...
    ATPPriceType price; ///委托价格N13(4)（必填）
    ATPEnforceFlagType enforce_flag; ///风险强制标识（已弃用）
    ATPConfirmIDType confirm_id; ///约定号（必填）
    ATPTrdSubTypeType trd_sub_type; ///是否受限（必填）0：非受限股份； 1：受限股份
    ATPPBUIDType counter_party_pbu_id; ///对手方PBUID（必填）
...
struct TRADE_API ATPReqBlockTradePricingOrderMsg
{
    ...
    ATPPriceType price; ///委托价格N13(4)（必填）
    ATPEnforceFlagType enforce_flag; ///风险强制标识（已弃用）
    ATPPricingType pricing; ///是否协议交易定价（必填）1=协议交易定价申报(约定号不用填) 2=协议交易定价成交申报(约定号必填且不可撤单）
...
struct TRADE_API ATPReqBlockTradeCancelOrderMsg
{
    ...
    ATPClientFeatureCodeType client_feature_code; ///终端识别码（该字段是否必填请咨询券商）
    ATPMarketIDType market_id; ///市场代码（非必填）
    ATPClOrdNOType orig_cl_ord_no; ///客户订单编号（必填），指示被撤消订单的ClOrdNO。
...
struct TRADE_API ATPReqCancelBatchOrderMsg
{
    ...
    ATPExtraData extra_data; ///扩展字段（非必填）
    ATPClientFeatureCodeType client_feature_code; ///终端识别码（该字段是否必填请咨询券商）
    ATPClOrdNOType parent_cl_ord_no; ///原始交易客户方（券商）订单编号，指示被撤消订单的ClOrdNO。（必填）
...
struct TRADE_API ATPReqReduceHoldingTransferOrderMsg
{
    ...
    ATPPasswordType password; ///客户密码（非必填）
    ATPExtraData extra_data; ///扩展字段（非必填）
    ATPClientFeatureCodeType client_feature_code; ///终端识别码（非必填）
    ATPSecurityIDType security_id; ///证券代码（必填）
    ATPQtyType order_qty; ///申报数量N15(2)（股票为股、基金为份、上海债券默认为张（使用时请务必与券商确认），其他为张）
    ATPEnforceFlagType enforce_flag; ///风险强制标识（已弃用）
    ATPAccountIDType counter_party_account_id; ///对手方股东账户（必填）
    ATPPBUIDType counter_party_pbu_id; ///对手方PBUID（必填）
...
struct TRADE_API ATPReqExtOptionFundTransfeOtherMsg
{
    ...
    ATPFundAccountIDType fund_account_id; ///资金账户ID（必填）
    ATPBranchIDType branch_id; ///营业部ID（必填）
    ATPFundAccountIDType account_id; ///证券账户ID（必填）
    ATPMarketIDType market_id; ///市场代码（必填）
    ATPCurrencyType currency; ///货币种类（非必填，默认值 "CNY"）
    ATPTransferWayType transfer_way; ///划拨方式（必填）：1-转入；2-转出；4-可划入资金查询
    ATPAmtType transfer_value; ///划拨金额 N18(4)（必填）
    ATPRemarkType remark; ///备注（必填）
    ATPPasswordType password; ///密码（非必填）
    ATPClientSeqIDType client_seq_id; ///用户系统消息序号（必填）
...
struct TRADE_API ATPReqExtQueryExerciseProfitLossMsg
{
    ...
    ATPMarketIDType market_id; ///市场代码（必填）
    ATPSecurityIDType security_id; ///证券代码（必填）
    ATPQtyType order_qty; ///行权申报数量N15(2)（必填）（股票为股、基金为份、上海债券默认为张（使用时请务必与券商确认），其他为张）
    ATPExtraData extra_data; ///扩展字段（非必填）
    ATPClientFeatureCodeType client_feature_code; ///终端识别码（该字段是否必填请咨询券商）
...
struct TRADE_API ATPReqHKStocksThroughAuctionOrderMsg
{
    ...
    ATPEnforceFlagType enforce_flag; ///风险强制标识（已弃用）
    ATPPriceType stop_px; ///止损价N13(4)（必填）
    ATPOrdTypeType ord_type; ///订单类型（必填）
    ATPQtyType min_qty; ///最低成交数量 N15(2)（已弃用）
    ATPMaxPriceLevelsType max_price_levels; ///最多成交价位数（已弃用）
    ATPTimeInForceType time_in_force; ///订单有效时间类型（已弃用）
    ATPLotTypeType lot_type; ///订单数量类型（必填），'1' = 零股订单，'2' = 整手订单
...
struct TRADE_API ATPReqHKVotingOrderMsg
{
    ...
    ATPSecurityIDType security_id; ///证券代码（必填）
    ATPMarketIDType market_id; ///市场代码（必填）
    ATPSideType side; ///买卖方向（必填）'1': 申报 'Q'：查询
    ATPEnforceFlagType enforce_flag; ///风险强制标识（已弃用）
    ATPAnnouncementNumberType announcement_number; ///公告编号（必填）
    ATPProxyFormNumberType proxy_form_number; ///议案编号（必填）
    ATPAffirmativeVotesType affirmative_votes; ///赞成数量（必填）
    ATPNegativeVotesType negative_votes; ///反对数量（必填）
    ATPAbstentionVotesType abstention_votes; ///弃权数量（必填）
...
struct TRADE_API ATPReqHKDesignationTransferOrderMsg
{
    ...
    ATPSecurityIDType security_id; ///证券代码（必填）
    ATPMarketIDType market_id; ///市场代码（必填）
    ATPSideType side; ///买卖方向（必填）'J'=DesignationTransfer，转托管 'K'=RevertDesignationTransfer，反向转托管
    ATPQtyType order_qty; ///申报数量N15(2)（必填）（股票为股、基金为份、上海债券默认为张（使用时请务必与券商确认），其他为张）
    ATPEnforceFlagType enforce_flag; ///风险强制标识（已弃用）
    ATPClOrdNOType orig_cl_ord_no; ///原委托的订单编号（必填）
    ATPDesignationTransTypeType designation_trans_type; ///转托管注册类型（必填）
    ATPEntitlementNumberType entitlement_number; ///权益编号（必填）
    ATPPBUIDType transferee_pbu_id; ///转托管，转入交易单元代码；反向转托管，转出交易单元（必填）
...
struct TRADE_API ATPReqHKCorporateActionOrderMsg
{
    ...
    ATPQtyType order_qty; ///申报数量N15(2)（必填）（股票为股、基金为份、上海债券默认为张（使用时请务必与券商确认），其他为张）
    ATPEnforceFlagType enforce_flag; ///风险强制标识（已弃用）
    ATPCorporateActionIDType corporate_action_id; ///公司行为代码（必填）
...
struct TRADE_API ATPReqHKCorporateActionCancelOrderMsg
{
    ...
    ATPExtraData extra_data; ///扩展字段（非必填）
    ATPClientFeatureCodeType client_feature_code; ///终端识别码（该字段是否必填请咨询券商）
    ATPSecurityIDType security_id; ///证券代码（非必填）
    ATPMarketIDType market_id; ///市场代码（非必填）
    ATPClOrdNOType orig_cli_ord_no; ///原委托的订单编号（必填）
...
struct TRADE_API ATPReqOptionCoveredTransUncoveredOrderMsg
{
    ...
    ATPBranchIDType branch_id; ///营业部ID
    ATPAccountIDType account_id; ///证券账户ID（必填）
    ATPSideType side; ///买卖方向（必填）买卖方向，备兑转普通填1，普通转备兑填2
    ATPPriceType price; ///委托价格N13(4)（必填）
    ATPQtyType qty; ///委托数量（必填）
    ATPSeqNumType client_seq_id; ///用户系统消息序号（必填）
    ATPClientFeatureCodeType client_feature_code; ///终端识别码（该字段是否必填请咨询券商）
    ATPOrderWayType order_way; ///委托方式（是否必填以及字典项请咨询券商）
    ATPUserInfoType user_info; ///用户私有信息（非必填）
...
struct TRADE_API ATPReqExtQueryCombinExerciseMaxOrderQtyMsg
{
    ...
    ATPClientFeatureCodeType client_feature_code; ///终端识别码（该字段是否必填请咨询券商）
    ATPMarketIDType market_id; ///市场代码（必填）
    std::vector<APIExerciseLegType> no_legs_array; ///行权多腿信息组合（必填）
...
struct TRADE_API ATPReqExternalQueryOptionCombinationMaxOrderQtyMsg
{
    ...
    ATPSecondaryOrderIDType secondary_order_id; ///组合编码（必填）
    ATPSideType side; ///买卖方向（必填）
    ATPLengthType no_legs; ///成份合约个数（必填）
    std::vector<APINoLegsOrderUnitType> no_legs_array; ///单腿信息组（必填）
...
struct TRADE_API ATPReqExternalInsTETransFundReqMsg
{
    ...
    ATPFundAccountIDType fund_account_id; ///资金账户ID（必填）
    ATPBranchIDType branch_id; ///营业部ID（必填）
    ATPAccountIDType fund_out_account_id; ///资金转出账户ID（必填）
    ATPMarketIDType fund_out_market_id; ///资金转出市场代码（必填）
    ATPAccountIDType fund_in_account_id; ///资金转入账户ID（必填）
    ATPMarketIDType fund_in_market_id; ///资金转入市场代码（必填）
    ATPAmtType value; ///划拨金额（必填）N18(4)
    ATPCurrencyType currency; ///货币种类（必填）
    ATPSeqNumType client_seq_id; ///用户系统消息序号（必填）
    ATPClientFeatureCodeType client_feature_code; ///终端识别码（该字段是否必填请咨询券商）
    ATPUserInfoType user_info; ///用户私有信息（非必填）
...
{
    ATPExtraData extra_data; ///扩展字段（非必填）
    ATPCustIDType cust_id; ///客户号ID（非必填）（cust_id、fund_account_id+branch_id两者必选填其一）
    ATPFundAccountIDType fund_account_id; ///资金账户ID（非必填）（cust_id、fund_account_id+branch_id两者必选填其一）
    ATPBranchIDType branch_id; ///营业部ID（非必填）（cust_id、fund_account_id+branch_id两者必选填其一）
    ATPPasswordType old_password; ///原密码（必填）
    ATPPasswordType password; ///新密码（必填）
    ATPOrderWayType order_way; ///委托方式（是否必填以及字典项请咨询券商）
    ATPSeqNumType client_seq_id; ///用户系统消息序号（必填）
    ATPUserInfoType user_info; ///用户私有信息（非必填）
    ATPClientFeatureCodeType client_feature_code; ///终端识别码（该字段是否必填请咨询券商）
...
struct TRADE_API ATPReqDeductionToBePaidOrderMsg
{
    ...
    ATPOrdTypeType order_type; ///订单类型（必填）
    ATPEnforceFlagType enforce_flag; ///风险强制标识（已弃用）
    ATPPBUIDType deduction_pbu; ///待清偿划扣证券的交易单元（必填）
    ATPAccountIDType deduction_account_id; ///待清偿划扣证券的证券账户（必填）
    ATPAccountTypeType deduction_account_type; ///待清偿划扣账户账户类型（必填）
...
struct TRADE_API ATPReqExternalQueryOptionSecurityInfoMsg
{
    ...
    ATPPasswordType password; ///客户密码（非必填）
    ATPClientFeatureCodeType client_feature_code; ///终端识别码（该字段是否必填请咨询券商）
    ATPSecurityIDType security_id; ///证券代码(合约编码)（非必填，填0表示查所有）
    ATPMarketIDType market_id; ///市场代码（非必填，填0表示查所有）
    ATPReportIndexType query_index; ///起始查询索引号（非必填）（默认值为0，表示从头开始查）
    ATPTotalNumType return_num; ///查询返回数量（非必填）（默认值为0，表示按能返回的最大数量返回，具体数量请咨询券商）
...
struct TRADE_API ATPReqOptionTradeOrderQueryMsg
{
    ...
    ATPExtraData extra_data; ///扩展字段（非必填）
    ATPClientFeatureCodeType client_feature_code; ///终端识别码（该字段是否必填请咨询券商）
    ATPMarketIDType market_id; ///市场代码（非必填）（默认值为0，表示查所有）
    ATPSecurityIDType security_id; ///证券代码（非必填）（默认值为空字符串，表示查所有）
    ATPBusinessTypeType business_type; ///业务类型（非必填）（默认值为0，表示查所有）
    ATPReportIndexType query_index; ///起始查询索引号（非必填）（默认值为0，表示从头开始查）
    ATPTotalNumType return_num; ///查询返回数量（非必填）（默认值为0，表示按能返回的最大数量返回，具体数量请咨询券商）
    ATPReturnSeqType return_seq; ///返回顺序，按时间排序（非必填）（默认值为1，表示按时间顺序排序）
...
struct TRADE_API ATPRspQueryMarginTradingPrivateAmoutResultMsg
{
    ...
    ATPExtraData extra_data; ///扩展字段
    ATPBranchIDType branch_id; ///营业部ID
    ATPAmtType private_amount; ///保底额度
    ATPAmtType private_amount_occupied; ///保底额度占用
    ATPAmtType public_amount_occupied; ///公用额度占用
    ATPFirstUsePrivateAmountType first_use_private_amount; ///是否先用保底额度
...
struct TRADE_API APISecuritiesLendingPrivateAmountUnit
{
    ...
    ATPSecurityIDType security_id; ///证券代码
    ATPQtyType private_amount; ///保底额度
    ATPQtyType private_amount_occupied; ///保底额度占用
    ATPQtyType public_amount_occupied; ///公用额度占用
    ATPFirstUsePrivateAmountType first_use_private_amount; ///是否先用保底额度
...
struct TRADE_API ATPReqBankSecuritiesFundTransferOtherMsg
{
    ...
    ATPMarketIDType market_id; ///市场代码（必填）
    ATPCurrencyType currency; ///货币种类（非必填）（默认值 "CNY"）
    ATPTransferWayType transfer_way; ///划拨方式（必填）：1-银衍转入；2-银衍转出
    ATPAmtType transfer_value; ///划拨金额 N18(4)（必填）
    ATPRemarkType remark; ///备注（必填）
    ATPPasswordType password; ///客户密码（非必填）
    ATPClientSeqIDType client_seq_id; ///用户系统消息序号（必填）
...
struct TRADE_API ApiNewShareInfoArray
{
    ...
    ATPPriceType price_upper_limit; ///发行价上限
    ATPPriceType price_lower_limit; ///发行价下限
    ATPQtyType purchase_unit; ///申购单位
    ATPQtyType max_purchase_qty; ///最大申购量
    ATPQtyType init_issued_total_qty; ///网上初始发行股数
...
struct TRADE_API ApiSecurityInfoArray
{
    ...
    ATPSecuritySymbolType security_symbol; ///申购代码名称
    ATPSecurityTypeType security_type; ///证券类别
    ATPPriceType prev_close_price; ///昨日收盘价
    ATPPriceType price_upper_limit; ///涨停价
    ATPPriceType price_lower_limit; ///跌停价
    ATPPriceTickType price_tick; ///最小变动价位
    ATPQtyType buy_qty_lower_limit; ///最小买数量单位
    ATPQtyType sell_qty_lower_limit; ///最小卖数量单位
    ATPBoolType day_trading; ///是否支持回转交易
    ATPBoolType has_price_limit; ///涨跌幅限制
    ATPSecurityStatusType security_status; ///证券状态
...
struct TRADE_API ATPReqAlgoParentOrderMsg
{
    ...
    ATPFundAccountIDType fund_account_id; ///资金账户ID（必填）
    ATPBranchIDType branch_id; ///营业部ID（必填）
    ATPAccountIDType account_id; ///账户ID（必填）
    ATPClientSeqIDType client_seq_id; ///用户系统消息序号（必填）
    ATPOrderWayType order_way; ///委托方式（是否必填以及字典项请咨询券商）
    ATPUserInfoType user_info; ///用户私有信息（非必填）
    ATPPasswordType password; ///客户密码（非必填）
    ...
    ATPClientFeatureCodeType client_feature_code; ///终端识别码（该字段是否必填请咨询券商）
    ATPSecurityIDType security_id; ///证券代码（必填）
    ATPMarketIDType market_id; ///市场ID （必填）
    ATPSideType side; ///买卖方向 （必填）
    ATPQtyType order_qty; ///母单数量N15(2)（必填，股票为股、基金为份、上海债券默认为张（使用时请务必与券商确认），其他为张）
    ATPPriceType price; ///母单的限价N13(4)（必填）;表示算法子单委托价格的买入上限或卖出下限；填0表示母单市价，即不限制子单价格上下限。
    ATPOrdTypeType order_type; ///订单类型 （必填），配合price一起使用
    ATPTimeInForceType time_in_force; ///订单有效时间类型 （必填）;'0'=当日有效
    ATPStringType algo_name; ///算法名称 （必填），例如：TWAP、VWAP等
    ATPTimeType start_time; ///算法开始时间 （必填，填0表示从当前时间开始）格式YYYYMMDDHHmmSSsss;例：20201124093000000
    ATPTimeType end_time; ///算法结束时间 （必填，填0表示结束时间由“执行时长”参数决定）格式YYYYMMDDHHmmSSsss;例：20201124150000000
    ATPAlgoDurationSecType duration_sec; ///算法执行长度(单位秒) （必填，当结束时间为0时，此参数生效），算法系统会自动剔除非交易时间。如开始时间对应9:00:00，此参数为300秒，对应的有效执行时间段是9:30:00~9:35:00
    ATPAlgoPartRateType part_rate; ///算法参与比例  N5(4)（必填），值域[0,10000]，如值2000表示量比20%，0或10000表示没有参与比例限制
    ATPStringType extra_params; ///算法扩展参数（非必填）;Json格式，如设置单笔委托最小金额为10000元，对应值: {“min_amount”:10000}
    ATPStringType basket_id; ///篮子ID（非必填）
    ATPAlgoBasketCountType basket_count; ///一篮子算法母单个数（非必填）
...
struct TRADE_API ATPReqExternalQueryAlgoParentMsg
{
    ...
    ATPPasswordType password; ///客户密码（非必填）
    ATPExtraData extra_data; ///扩展字段（非必填）
    ATPClientFeatureCodeType client_feature_code; ///终端识别码（保留）。【该字段启用需特殊版本】（非必填）
    ATPClOrdNOType parent_cl_ord_no; ///算法母单编号（必填）,填0查所有
    ATPMarketIDType market_id; ///证券所属市场（必填）,填0查所有
    ATPSecurityIDType security_id; ///证券代码（必填）,填空查所有
    ATPBusinessTypeType business_type; ///业务类型（必填）,填0查所有
    ATPSideType side; ///买卖方向（必填）,填'0'查所有
    ATPReportIndexType query_index; ///起始查询索引号（必填）,填0表示从头开始查
    ATPTotalNumType return_num; ///指定查询返回数量（必填）,填 0表示不指定
    ATPReturnSeqType return_seq; ///返回顺序，按时间排序（必填）,例如，1=按时间正序，2=按时间倒序
...
struct TRADE_API ATPRspAlgoParentExecReportMsg
{
    ...
    ATPQtyType cum_sent_qty; ///累计成交数量 + 在途数量
    ATPQtyType cum_qty; ///累计成交数量
    ATPPriceType cum_avg_price; ///累计成交均价
...
struct TRADE_API ATPRspExternalQueryResultAlgoParentMsg
{
    ATPCustIDType cust_id; ///客户号ID
    ATPFundAccountIDType fund_account_id; ///资金账户ID
    ATPBranchIDType branch_id; ///OrigId+营业部ID 
    ATPAccountIDType account_id; ///账户ID
    ATPClientSeqIDType client_seq_id; ///用户系统消息序号
    ATPQueryResultCodeType query_result_code; ///查询结果代码 注：V217添加
    ATPUserInfoType user_info; ///用户私有信息 注：V212添加
    ATPExtraData extra_data; ///扩展字段
...
struct TRADE_API ATPReqCancelAlgoParentOrderMsg
{
    ...
    ATPExtraData extra_data; ///扩展字段（非必填）
    ATPClientFeatureCodeType client_feature_code; ///终端识别码（该字段是否必填请咨询券商）
    ATPClOrdNOType orig_parent_cl_ord_no; ///原算法母单编号（必填）
...
struct TRADE_API ATPReqExternalQueryAlgoOrderMsg
{
    ...
    ATPPasswordType password; ///客户密码（非必填）
    ATPExtraData extra_data; ///扩展字段（非必填）
    ATPClOrdNOType parent_cl_ord_no; ///算法母单编号 (必填)
    ATPClOrdNOType cl_ord_no; ///客户订单编号,同一个客户当天唯一,填0查所有
    ATPMarketIDType market_id; ///证券所属市场（非必填）（默认值为0，表示查所有，一户两地账户表示查单个证券账户下所有）
    ATPSecurityIDType security_id; ///证券代码证券代码（非必填）（默认值为空字符串，表示查所有，一户两地账户表示查单个证券账户下所有）
    ATPBusinessTypeType business_type; ///业务类型业务类型（非必填）（默认值为0，表示查所有，一户两地账户表示查单个证券账户下所有）
    ATPSideType side; ///买卖方向（非必填）
    ATPOrderQueryConditionType order_query_condition; ///委托查询条件（非必填）（默认值为0，表示查所有，一户两地账户表示查单个证券账户下所有）
    ATPReportIndexType query_index; ///起始查询索引号（非必填）（默认值为0，表示从头开始查）
    ATPTotalNumType return_num; ///查询返回数量（非必填）（默认值为0，表示按能返回的最大数量返回，具体数量请咨询券商）
    ATPReturnSeqType return_seq; ///返回顺序，按时间排序（非必填）（默认值为1，表示按时间顺序排序）
