// code_generator_tag
// requests:37421335b23d60b36e62f18d19a123ca template:f9aa7b0012499bf10e4895bdaf2c877c code:0d5309cf437d9aad1aaade8547541ad6
...
class TRADE_API ATPTradeAPI
{
    ...
    public:
	    ...
	    */
	     ...
	     *	
	     */
	    ATPRetCodeType Connect(const ATPConnectProperty& property, ATPTradeHandler* handler);
         ...
         *
        */
	    ATPRetCodeType ConnectCust(const ATPConnectCustProperty& property, ATPTradeHandler* handler);
	    ...
	    */ 
         ...
         * @business   cash,margin
         * @type       query
         * @note       相比于成交查询函数，增强成交查询函数在3.1.5版本之前支持按客户合同号查询单条记录，并且支持撤单和撤单失败的响应查询；在3.1.5版本及之后与成交查询函数无差别
         */
         ...
         * @business   cash,margin
         * @type       query
         * @note       v3.1.6之前版本是通过待查证券账户组合（account_id_array）中的信息进行查询判断；如果组合内填入account_id，但是没有传入fund_accout_id和branch_id，则需要外层传入market_id才能查询到对应市场的持仓；如果组合内同时填入account_id、fund_accout_id和branch_id,则不校验外层的market_id直接查询对应市场的持仓；如果需要查询多个市场的持仓则需要传入多个待查证券账户组合（account_id_array）；v3.1.6及后续版本支持不传入待查证券账户组合（account_id_array）信息，直接根据外层的fund_accout_id和branch_id查询出所有持仓
         */
         ...
         * @business   cash,margin
         * @type       query
         * @note       相比于资金查询函数，增强资金查询函数支持币种字段填空查询所有币种的资金
         */
         ...
         * @business   cash
         * @type       order
         * @note       批量委托类型为单代码等量拆单时：单笔上限字段（max_order_qty）为必填项，其值为等量拆单的每笔订单数量；批量委托类型为单代码递减拆单时：递减数量字段（decrease_qty）为必填项，其值为每笔的减数；批量委托类型为多代码批量报单时：多证券委托组合为必填项
         */
         ...
         * @business   algo
         * @type       order
         * @note       现阶段支持沪深市场主板、科创板、创业板的股票、债券、基金的竞价买卖业务；详细可见开发指南
         */
